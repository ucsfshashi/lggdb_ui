...
export default class AlterationRowEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
	    	 ...
	    	 showLoading:false,
	    	 isNewPatient:(this.props.data['AlterationRow.gene'] == undefined)
		};
	...
	async saveInServer ()  {
		...
		const {config,mrn,loginContext} = this.props;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'AlterationRow changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save AlterationRow changes'});
	           this.setState({successMessage:null});
	       }
