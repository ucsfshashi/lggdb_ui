...
export default class AuditoryAttentionView extends React.Component {
	...
	async extractData () {
	    ...
	    var path ='Patient/'+loginContext.mrn+'/LoglioCognitive/'+this.props.parentInfo["LoglioCognitive.testingDate"]+'/AuditoryAttention';
   ...
   async saveInServer ()  {
		  ...
		  const {config,mrn,loginContext} = this.props;
	       ...
	       if(rInfo && rInfo.data === true) {
	       	this.setState({successMessage:'AuditoryAttention changes saved successfully'});
	       	this.setState({errorMessage:null});
	       } else {
	       	this.setState({errorMessage:'Failed to save AuditoryAttention changes'});
