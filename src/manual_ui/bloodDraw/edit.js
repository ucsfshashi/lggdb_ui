...
export default class BloodDrawEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
	    	 ...
	    	 showLoading:false,
	    	isNewPatient:(this.props.data['BloodDraw.bdDate'] == undefined)
		};
	...
	async saveInServer ()  {
		...
		const {config,mrn,loginContext} = this.props;
		   ...
		   var path ='Patient/'+loginContext.mrn+'/Epidemiology/'+this.props.parentId+'/BloodDraw';
		   var data = this.state.data;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'BloodDraw changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save BloodDraw changes'});
	           this.setState({successMessage:null});
	       }
