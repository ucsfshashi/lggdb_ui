...
export default class CgpAlterationRowEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
	    	 ...
	    	 showLoading:false,
	    	 isNewPatient:(this.props.data['CgpAlterationRow.gene'] == undefined)
		};
	...
	async saveInServer ()  {
		...
		const {config,mrn,loginContext} = this.props;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'CgpAlterationRow changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save CgpAlterationRow changes'});
	           this.setState({successMessage:null});
	       }
