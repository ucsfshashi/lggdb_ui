...
export default class ClinicalEvaluationEdit  extends React.Component {
	constructor(props) {
	    super(props);
	    ...
	    this.state = {
    	   ...
    	   showLoading:false,
    	   isNewPatient:(this.props.data['ClinicalEvaluation.clinicalEvaluationDate'] == undefined)
    	};
	};
	...
	async saveInServer ()  {
		...
		const {config,mrn,loginContext} = this.props;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'ClinicalEvaluation changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save ClinicalEvaluation changes'});
	           this.setState({successMessage:null});
	       }
