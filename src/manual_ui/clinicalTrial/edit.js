...
export default class ClinicalTrialEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
	    	 ...
	    	 showLoading:false,
	    	 isNewPatient:(this.props.data['ClinicalTrial.onCoreProtocolNumber'] == undefined)
		};
	...
	async saveInServer ()  {
      ...
      this.setState({showLoading:true});
	   ...
	   var path ='Patient/'+loginContext.mrn+'/ClinicalTrial';
	   var data = this.state.data;
      ...
      if(rInfo && rInfo.data === true) {
   	    this.setState({successMessage:'ClinicalTrial changes saved successfully'});
       	this.setState({errorMessage:null});
       	this.props.goBackToList(null);
       	
      } else {
   	   this.setState({errorMessage:'Failed to save ClinicalTrial changes'});
          this.setState({successMessage:null});
      }
	...
	render() {
		...
		return (<PatientCardView
		        ...
		        keyColumn={'onCoreProtocolNumber'}
