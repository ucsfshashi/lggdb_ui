...
export default class CognitionEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
    	   ...
    	   showLoading:false,
    	   isNewPatient:(this.props.data['Cognition.timeLine'] == undefined)
		};
	...
	async saveInServer ()  {
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'Patient Cognition changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save Patient Cognition changes'});
	           this.setState({successMessage:null});
	       }
