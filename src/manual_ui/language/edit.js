...
export default class LanguageEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
    	   ...
    	   showLoading:false,
    	   isNewPatient:(this.props.data['Language.timeLine'] == undefined)
		};
	...
	async saveInServer ()  {
		   ...
		   var path ='Patient/'+loginContext.mrn+'/Language';
		   var data = this.state.data;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'Patient Language changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save Patient Language changes'});
	           this.setState({successMessage:null});
	       }
