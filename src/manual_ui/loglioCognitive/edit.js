...
export default class LoglioCognitiveEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
    	   ...
    	   showLoading:false,
    	   isNewPatient:(this.props.data['LoglioCognitive.timeLine'] == undefined)
		};
	...
	async saveInServer ()  {
		   ...
		   const {config,mrn,loginContext} = this.props;
 	       ...
 	       if(rInfo && rInfo.data === true) {
 	    	   this.setState({successMessage:'Patient LoglioCognitive changes saved successfully'});
 	        	this.setState({errorMessage:null});
 	        	this.props.goBackToList(null);
 	        	
 	       } else {
 	    	  this.setState({errorMessage:'Failed to save Patient LoglioCognitive changes'});
 	           this.setState({successMessage:null});
 	       }
