...
export default class McGurkEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
    	   ...
    	   showLoading:false,
    	   isNewPatient:(this.props.data['McGurk.timeLine'] == undefined)
		};
	...
	async saveInServer ()  {
		   ...
		   var path ='Patient/'+loginContext.mrn+'/McGurk';
		   var data = this.state.data;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'Patient McGurk changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save Patient McGurk changes'});
	           this.setState({successMessage:null});
	       }
