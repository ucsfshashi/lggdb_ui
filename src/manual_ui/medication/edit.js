...
export default class MedicationEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
	    	 ...
	    	 showLoading:false,
	    	 isNewPatient:(this.props.data['Medication.medicationStartDate'] == undefined)
		};
	...
	async saveInServer ()  {
      ...
      this.setState({showLoading:true});
	   ...
	   var path ='Patient/'+loginContext.mrn+'/Medication';
	   var data = this.state.data;
      ...
      if(rInfo && rInfo.data === true) {
   	    this.setState({successMessage:'Medication changes saved successfully'});
       	this.setState({errorMessage:null});
       	this.props.goBackToList(null);
       	
      } else {
   	   this.setState({errorMessage:'Failed to save Medication changes'});
          this.setState({successMessage:null});
      }
