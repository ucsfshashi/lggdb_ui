...
export default class OtherTreatmentEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
    	   ...
    	   showLoading:false,
    	   isNewPatient:(this.props.data['OtherTreatment.otName'] == undefined)
		};
	...
	async saveInServer ()  {
		   ...
		   var path ='Patient/'+loginContext.mrn+'/OtherTreatment';
		   var data = this.state.data;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'Patient OtherTreatment changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save Patient OtherTreatment changes'});
	           this.setState({successMessage:null});
	       }
