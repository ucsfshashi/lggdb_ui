...
export default class SensoryEdit extends React.Component {
	...
	constructor(props) {
	    ...
	    this.state = {
    	   ...
    	   showLoading:false,
    	   isNewPatient:(this.props.data['Sensory.timeLine'] == undefined)
		};
	...
	async saveInServer ()  {
		   ...
		   var path ='Patient/'+loginContext.mrn+'/Sensory';
		   var data = this.state.data;
	       ...
	       if(rInfo && rInfo.data === true) {
	    	    this.setState({successMessage:'Patient Sensory changes saved successfully'});
	        	this.setState({errorMessage:null});
	        	this.props.goBackToList(null);
	        	
	       } else {
	    	   this.setState({errorMessage:'Failed to save Patient Sensory changes'});
	           this.setState({successMessage:null});
	       }
